...
def get_new_data():
    ...
    while True:
        ...
        try:
            if macros_text == "":
                logging.getLogger(__name__).info("Nothing was entered, please try again.")
            else:
                macros_list = macros_text.split(" ")
                if len(macros_list) != 4:
                    logging.getLogger(__name__).info("Invalid number of entries for macros entry, please try again")
                    continue
                all_values = []
                for value in macros_list:
                    all_values.append(int(value))
                all_values.append((all_values[0] * 4) + (all_values[1] * 9) + (all_values[2] * 4))
                logging.getLogger(__name__).info('Nutrition text received: %s' % all_values)
                return all_values
        except ValueError:
